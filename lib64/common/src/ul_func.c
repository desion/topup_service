...
void  *ul_calloc(size_t n, size_t size)
{
	void *ptr = NULL;
	if (n <= 0 || size <= 0)
		slog_write(LL_WARNING, "calloc(%zu,%zu) call failed.Parameter was error.",
				n, size);
	ptr = calloc(n, size);
	if (NULL == ptr)
		slog_write(LL_WARNING, "calloc(%zu,%zu) call failed.error[%d] info is '%s'.",
...
void *ul_malloc(size_t size)
{
	...
	if (NULL == ptr)
		slog_write(LL_WARNING, "malloc(%zu) call failed.error[%d] info is '%s'.",
...
void *ul_realloc(void *ptr, size_t size)
{
	...
	if (NULL == ptr0)
		slog_write(LL_WARNING, "realloc(%zu) failed.error[%d] info is '%s'.",
				size, errno, strerror(errno));
	return	ptr0;
}
...
int ul_open(const char *pathname, int oflag, mode_t mode)
{
	...
	if (-1 == fd)
		slog_write(LL_WARNING,"open(%s,%d,%d) call failed.error[%d] info is '%s'.",
...
ssize_t ul_read(int fd, void *ptr, size_t nbytes)
{
	...
	if (-1 == n)
		slog_write(LL_WARNING,"read(%d,%zu) call failed.error[%d] info is '%s'.",
...
ssize_t ul_write(int fd, void *ptr, size_t nbytes)
{
	...
	if (-1 == bytes)
		slog_write(LL_WARNING, "write(%d,%zu) call failed.error[%d] info is '%s'.", 
				fd, nbytes, errno, strerror(errno));
	return bytes;
}
...
int ul_close(int fd)
{
	int val = close(fd);
	if (-1 == val)
		slog_write(LL_WARNING, "close(%d) call failed.error[%d] info is '%s'.",
...
int ul_fcntl(int fd, int cmd, int arg)
{
	...
	if (-1 == n)
		slog_write(LL_WARNING, "fcntl(%d,%d,%d) call failed.error[%d] info is '%s'.",
...
int	ul_dup(int oldfd)
{
	int	fd = dup(oldfd);
	if (-1 == fd)
		slog_write(LL_WARNING, "dup(%d) call failed.error[%d] info is '%s'.",
...
int	ul_dup2(int oldfd, int newfd)
{
	...
	if (-1 == fd)
		slog_write(LL_WARNING, "dup2(%d) call failed.error[%d] info is '%s'.",
				oldfd, errno, strerror(errno));
	return fd;
}
...
off_t ul_lseek(int fildes, off_t offset, int whence)
{
	...
	if (-1 == pos)
		slog_write(LL_WARNING, "lseek(%d,%ld,%d) call failed.error(%d) info is %s.",
...
int ul_pipe(int fds[2])
{
	int	val = pipe(fds);
	if (-1 == val)
		slog_write(LL_WARNING, "pipe() call failed.error(%d) info is %s.",
...
int ul_stat(const char *file_name, struct stat *buf)
{
	...
	if(-1 == val)
		slog_write(LL_WARNING, "stat(%s) call failed.error[%d] info is '%s'.",
...
int ul_fstat(int filedes, struct stat *buf)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fstat(%d) call failed.error[%d] info is '%s'.",
...
int	ul_unlink(const char *pathname)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "unlink(%s) call failed.error[%d] info is '%s'.",
...
int ul_fchmod(int fildes, mode_t mode)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fchmod(%d,%d) call failed,error[%d] info is '%s'.",
...
int	ul_chdir(const char *path)
{
	int	val = chdir(path);
	if (-1 == val)
		slog_write(LL_WARNING, "chdir(%s) call failed.error[%d] info is '%s'.",
...
int ul_fchdir(int fildes)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fchdir(%d) call failed,error[%d] info is '%s'.",
...
int ul_fchown(int fildes, uid_t owner,gid_t group)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fchown(%d,%d,%d) call failed,error[%d] info is '%s'.",
...
int	ul_truncate(const char *path, off_t length)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING,"truncate(%s,%ld) call failed.error[%d] info is '%s'.",
...
int	ul_ftruncate(int fd, off_t length)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "ftruncate(%d,%ld) call failed.error[%d] info is '%s'.",
...
long ul_fpathconf(int filedes, int name)
{
	...
	if (-1 == val) {
		if (error != errno)
			slog_write(LL_WARNING, "fpathconf(%d,%d) call failed.error[%d] info is '%s'.",
...
long ul_pathconf(char *path,int name)
{
	...
	if (-1 == val) {
		if (error != errno)
			slog_write(LL_WARNING, "pathconf(%s,%d) call failed.error[%d] info is '%s'.",
...
char *ul_mmap(void *start, size_t length, int prot , int flags, int fd, off_t offset)
{
	...
	if ((void *)-1 == ptr)
		slog_write(LL_WARNING, "mmap(%p,%zu,%d,%d,%d,%ld) call failed.error[%d] info is '%s'.",
...
int ul_munmap(void *start, size_t length)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "munmap(%p,%zu) call filed.error[%d] info is '%s'.",
...
FILE *ul_fopen(const char *filename, const char *mode)
{
	...
	if (NULL == fp)
		slog_write(LL_WARNING, "fopen(%s,%s) call was failed.error[%d] info is '%s'.",
...
int ul_fclose(FILE *fp)
{
	...
	if (val != 0) {
		val = -1;
		slog_write(LL_WARNING, "fclose() call was filed.error[%d] info is '%s'.",
				errno, strerror(errno));
	}
	return  val;
}
...
{   
	...
	if (NULL == fp)
		slog_write(LL_WARNING, "fdopen(%d,%s) call was failed.error[%d] info is '%s'.",
...
FILE *ul_freopen(const char *path, const char *mode, FILE *stream)
{
	...
	if (NULL == fp)
		slog_write(LL_WARNING, "freopen(%s,%s) call failed.error[%d] info is '%s'.",
...
char *ul_fgets(char *ptr, int n, FILE *stream)
{
	...
	if (NULL == rptr && ferror(stream))
		slog_write(LL_WARNING,"fgets(%d) call failed,error[%d] info is '%s'.",
...
int ul_fputs(const char *ptr, FILE *stream)
{
	int	val = EOF;
	if (NULL == ptr) {
		slog_write(LL_WARNING, "Parameter 'ptr' was NULL.");
		return EOF;
	}
	if ((val = fputs(ptr,stream)) == EOF)
		slog_write(LL_WARNING, "fputs(len=%zu) call failed.error[%d] info is '%s'.",\
...
int ul_fseek( FILE *stream, long offset, int whence)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fseek(%ld,%d) call failed.error[%d] info is '%s'.",
...
long ul_ftell(FILE *stream)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "ftell() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_fgetpos(FILE *stream, fpos_t *pos)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fgetpos() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
//*******************************
...
int ul_fsetpos(FILE *stream, fpos_t *pos)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "fsetpos call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_remove(const char *pathname)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "remove(%s) call failed.error[%d] info is '%s'.",
				pathname, errno, strerror(errno));
	return val;
}
...
int ul_rename(const char *oldpath, const char *newpath)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "rename(%s,%s) call failed.error[%d] info is '%s'.",
...
FILE *ul_tmpfile (void)
{
	...
	if (NULL == fp)
		slog_write(LL_WARNING, "tmpfile() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return fp;
}
...
FILE *ul_popen(const char *command, const char *type)
{
	...
	if (NULL == fp)
		slog_write(LL_WARNING, "popen(%s,%s) call failed.error[%d] info is '%s'.",
...
int ul_pclose(FILE *stream)
{
	...
	val = pclose(stream);
	if (-1 == val)
		slog_write(LL_WARNING, "pclose() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
DIR *ul_opendir(const char *path)
{
	...
	if (NULL == dirp)
		slog_write(LL_WARNING, "opendir(%s) call failed.error[%d] info is '%s'.",
				path, errno, strerror(errno));
	return dirp;
}
...
struct dirent *ul_readdir(DIR *dir)
{
	...
	if (NULL == d)
		slog_write(LL_WARNING, "readdir() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return	d;
}
...
int	ul_closedir(DIR *dirp)
{
	...
	if (dirp == NULL || (val = closedir(dirp))== -1 )
		slog_write(LL_WARNING, "closedir() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_kill(pid_t pid, int sig)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "kill(%d,%d) call failed.error[%d] info is '%s'.",
				pid, sig, errno, strerror(errno));
	if (val == 0 && pid == -1 )
		slog_write(LL_WARNING, "kill process(pid == -1 ) executed successfully.");
	return val;
}
...
int ul_raise(int sig)
{
	int val = raise(sig);
	if (val != 0)
		slog_write(LL_WARNING, "raise(%d) call failed.error[%d] info is '%s'.",
...
int ul_sigemptyset(sigset_t *set)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigemptyset() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_sigfillset(sigset_t *set)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigfillset() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_sigaddset(sigset_t *set, int signum)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigaddset(%d) call failed.error[%d] info is '%s'.",
...
int ul_sigdelset(sigset_t *set, int signum)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigdelset(%d) call failed.error[%d] info is '%s'.",
				signum, errno, strerror(errno));
	return	val;
}
...
int ul_sigismember(const sigset_t *set, int signum)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigismember(%d) call failed.error[%d] info is '%s'.",
				signum, errno, strerror(errno));
	return val;	
}
...
int ul_sigaction(int signo,const struct sigaction *act,struct sigaction *oact)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigaction(%d) call failed.error[%d] info is '%s'.",
...
int ul_sigprocmask(int how, const sigset_t *set, sigset_t *oldset)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigprocmask(%d) call failed.error[%d] info is '%s'.",
...
int ul_sigpending(sigset_t *set)
{
	...
	if (-1 == val )
		slog_write(LL_WARNING, "sigpending() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_sigsuspend(const sigset_t *mask)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "sigsuspend() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
pid_t ul_fork(void)
{
	pid_t val = fork();
	if (-1 == val)
		slog_write(LL_WARNING, "fork() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;	
}
...
int ul_execve (const char *filename, char *const argv [], char *const envp[])
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "execve(%s) call failed.error[%d] info is '%s'.",
...
pid_t ul_wait(int *status)
{
	int	val = wait(status);
	if (val <= 0)
		slog_write(LL_WARNING, "wait() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
pid_t ul_waitpid(pid_t pid, int *status, int options)
{
	...
	if (val <= 0 )
		slog_write(LL_WARNING, "waitpid(%d,%d) call failed.error[%d] info is '%s'.",
...
int ul_setpgid(pid_t pid, pid_t pgid)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "setpgid(%d,%d) call failed.error[%d] info is '%s'.",
...
/* comment by Chen Jingkai at 2002:04:05:11:00:00 
   ...
   slog_write(LL_WARNING,"getpgid(%d) call failed.error[%d] info is '%s'.",pid,errno,strerror(errno));
   return	val;
   }
 */
//*********************************************
...
int ul_setpgrp(void)
{
	int	val = setpgrp();
	if (-1 == val)
		slog_write(LL_WARNING, "setpgrp() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_mkfifo(const char *pathname, mode_t mode)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "mkfifo(%s,%d) call failed.error[%d] info is '%s'.",
...
key_t ul_ftok(char *pathname, char proj)
{
	...
	if (-1 == key)
		slog_write(LL_WARNING, "ftok(%s,%c) call failed.error[%d] info is '%s'.",
...
int ul_shmget(key_t key, size_t size, int shmflg) //man 里第2个参数是int 可真正的头文件里却是size_t，这里使用size_t
{
	...
	if (-1 == id)
		slog_write(LL_WARNING, "shmget(%d,%zu,%d) call failed.error[%d] info is '%s'.",
...
void *ul_shmat (int shmid, const void *shmaddr, int shmflg )
{
	...
	if ((void *)(-1) == ptr)
		slog_write(LL_WARNING, "shmat(%d,%d) call failed.error[%d] info is '%s'.",
...
int	ul_shmdt (const void *shmaddr)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "shmdt() call failed.error[%d] info is '%s'.",
				errno, strerror(errno));
	return val;
}
...
int ul_shmctl(int shmid, int cmd, struct shmid_ds *buf)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "shmctl(%d,%d) call failed.error[%d] info is '%s'.",
...
int ul_semget( key_t key, int nsems, int semflg )
{
	...
	if (-1 == id)
		slog_write(LL_WARNING, "semget(%d,%d,%d) call failed.error[%d] info is '%s'.",
...
int ul_semctl (int semid, int semnum, int cmd, union semun arg)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "semctl(%d,%d,%d) call failed.error[%d] info is '%s'.",
...
int ul_semop( int semid, struct sembuf *sops, unsigned nsops )
{
	...
	while ((val = semop(semid,sops,nsops)) == -1) {
		...
		slog_write(LL_WARNING, "semop(%d) call failed.error[%d] info is '%s'.",
...
int ul_msgget(key_t key, int msgflg)
{
	...
	if (-1 == id)
		slog_write(LL_WARNING, "msgget(%d,%d) call failed.error[%d] info is '%s'.",
...
int ul_msgctl(int msqid, int  cmd, struct msqid_ds *buf)
{
	...
	if (-1 == val)
		slog_write(LL_WARNING, "msgctl(%d,%d) call failed.error[%d] info is '%s'.",
...
//******************************************************************* 
int ul_msgsnd(int msqid, void *msgp, size_t msgsz, int msgflg)
{
	int	nbyte = msgsnd(msqid, msgp, msgsz, msgflg);
	if (-1 == nbyte)
		slog_write(LL_WARNING, "msgsnd(%d,%zu,%d) call failed.error[%d] info is '%s'.",
				msqid, msgsz, msgflg, errno, strerror(errno));
	return nbyte;
}

int ul_strlcpy(char * dest, const char * src, size_t n)
{
...
//******************************************************************* 
ssize_t ul_msgrcv(int msqid, void *msgp, size_t msgsz, long msgtyp, int msgflg)
{
	ssize_t	nbyte = msgrcv(msqid, msgp, msgsz, msgtyp, msgflg);
	if (-1 == nbyte)
		slog_write(LL_WARNING, "msgrcv(%d,%zu,%ld,%d) call failed.error[%d] info is '%s'.",
