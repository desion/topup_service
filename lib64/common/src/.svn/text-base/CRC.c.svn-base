...
static int g_table16 = 0;		//CRC16表存在状态，0--不存在，1--存在
unsigned long g_Table_CRC16[256]; // CRC 表
static int g_table32 = 0;		//CRC32表存在状态，0--不存在，1--存在
unsigned long g_Table_CRC32[256]; // CRC 表
...
*
	...
	参 数 说 明：	aData[in]:待校验数据
	  ...
	  返回值 说明：	循环冗余校验结果
*******************************************************************/
unsigned short RunCRC16( const unsigned char * aData, unsigned long aSize, unsigned short aPoly )
{
    unsigned long  i;
	...
	if(g_table16 == 0)
	{
		BuildTable16( aPoly, g_Table_CRC16 );
		g_table16 = 1;
	}
    
    for ( i = 0; i < aSize; i++ )
    {
        nAccum = ( nAccum << 8 ) ^ ( unsigned short )g_Table_CRC16[( nAccum >> 8 ) ^ *aData++];
    }
    return nAccum;
}
...
*
	...
	参 数 说 明：	aData[in]:待校验数据
	  ...
	  返回值 说明：	循环冗余校验结果
*******************************************************************/
unsigned long RunCRC32( const unsigned char * aData, unsigned long aSize, unsigned long aPoly )
{
    unsigned long i;
	...
	if(g_table32 == 0)
	{
		BuildTable32( aPoly, g_Table_CRC32 );
		g_table32 = 1;
	}
    
    for ( i = 0; i < aSize; i++ )
    {
        nAccum = ( nAccum << 8 ) ^ g_Table_CRC32[( nAccum >> 24 ) ^ *aData++];
    }
    return nAccum;
}

//crc哈希算法
uint64_t RunCRC64(void *buf, size_t len, uint8_t type)
{
	uint64_t crc = crctab64[type];
	uint8_t *p = (uint8_t*) buf;

	while (len--)
	{
		crc = crctab64[(crc ^ *(p++)) & 0xff] ^ (crc >> 8);
	}

	return crc;
}

int CRC64(void *buf, size_t len, uint32_t * sign1, uint32_t * sign2)
{
	if (buf == NULL || sign1 == NULL || sign2 == NULL)
	{
		return -1;
	}

	uint64_t crc = RunCRC64(buf, len, 0);
	(*sign1) = (uint32_t) (crc & 0xffffffff);
	(*sign2) = (uint32_t) (crc >> 32);
	return 0;
}
