...
int TopupImpl::HandleRequest(const TopupRequest& request, string &result){
    ...
    if(uri != NULL){
        ...
        if(m_interface == NULL){
            fprintf(stderr, "can't get interface from uri %s\t%d\n", uri, m_topup_info->seqid);
            return 1; 
        }
    }
    //根据调用的URI参数判断调用的相应接口
...
int TopupImpl::TmallCharge(string &response){
    ...
    TP_WRITE_LOG(m_topup_info, "\tPRODUCT:%d v:%d o:%d p:%d",check_product, m_topup_info->qs_info.value,
    ...
    //tsc is error
        ...
        TP_WRITE_LOG(m_topup_info, "\t(TmallCharge) product tsc invalid %s", INVALID_ORDER);    
        return 3;
    }
    //TODO 选择最优的渠道
...
int TopupImpl::TmallNotify(){
    int retry = 5;
    int notify_status = 0;
    //notify tmall order status just 5 times
...
int TopupImpl::check_tsc(const char* phone ,int op,int province)
{
